...
void runSineInference(float x) {
    ...
    float predictedY = dequantizeInt8ToFloat(
        output->data.int8[0],
        output
    );
    float actualY = sin(x);
...
void setup() {
    ...
    Serial.println("ESP32 TFLite Micro Hello World started.");
    printMemoryInfo();
    setupModel();
    Serial.println("Running the deployed sine model. No external hardware required.");
}
